...
import { ElevenLabTtsService } from './eleven-labs.js';
...
export class TTSService {
    constructor(ttsService, voiceId, logger) {
        ...
        switch (ttsService || config.tts.provider.toLowerCase()) {
            ...
            case 'elevenlabs':
                ...
                this.service = new ElevenLabTtsService(voiceId,config);
                break;
            case 'azure':
                this.logger.info('Using Azure Speech for TTS');
                this.service = new AzureTTSService(voiceId,config);
                break;
            case 'openazure':
                this.logger.info('Using Azure Speech for TTS');
    ...
    async synthesize(text, callId) {
        return await this.service.synthesize(text, callId);
    }
}
